...
void ds_kh::in()
{
	...
	for (int i = 1; i <= sl; i++)
	{
		gotoXY(12, 8 + 2*i - 2); cout << i;
		//cout << "Ho va ten: " << n[i].getName() << endl;
		//cout << "SDT: " << n[i].get_sdt() << endl;
		gotoXY(32, 8 + 2 * i - 2); cout << n[i].getName();
		gotoXY(52, 8 + 2 * i - 2); cout << n[i].get_sdt();
		gotoXY(72, 8 + 2 * i - 2); cout << d.demDh(n[i].get_sdt());
		gotoXY(92, 8 + 2 * i - 2); cout << d.doanhthu(n[i].get_sdt());
